...
CREATE TABLE documents_tools (
    id BIGINT PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
    document_id BIGINT NOT NULL REFERENCES documents(id),
    tool_id BIGINT NOT NULL REFERENCES tools(id),
    created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
    UNIQUE(document_id, tool_id)
);
...
-- vector is 1536 dimension to match openai's embeddings
...
create policy "Public can query documents"
...
create policy "Public can query document sections"
